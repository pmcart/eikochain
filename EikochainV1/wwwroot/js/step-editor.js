...
$(document).ready(() => {
    ...
    if (window.location.href.toLowerCase().indexOf("view") > -1)
        isPreview = true;
    ...
    if (window.location.href.indexOf("Add") > -1) {
        ...
        $(".nav-step").each(function () {
            var tabId = this.id;
            $(this).prepend(`<i class="fa-solid fa-ellipsis-vertical menuStepIcon tabIcon editorElement" id="dropdownMenuButton" style="color: ${ eikochainLogoColor }" data-bs-toggle="dropdown" aria-expanded="false"></i>
            ...
            $(this).find(".tabItem").append(`
                <i data-targetstep="#${tabId}" class="fa-solid fa-caret-up fa-2xl moveStepUpIcon tabIcon editorElement" style="color: ${eikochainLogoColor}"></i>
                <i data-targetstep="#${tabId}" class="fa-solid fa-caret-down fa-2xl moveStepDownIcon tabIcon editorElement" style="color: ${eikochainLogoColor}"></i>
            `)
        })

        $(".stepContainerRow").each(function () {
            var rowId = this.id;
            console.log(rowId)
            $(this).addClass("resizableRow");

            $(this).append(`
                <i data-targetrow="${rowId}" class="addToRowBtn fa fa-plus-circle editRowBtn"></i>
                <i data-targetrow="${rowId}" class="rowDeleteBtn fa-solid fa-trash-can editRowBtn"></i>
                <i data-targetrow="${rowId}" class="arrowBtn arrowRowBtnUp fa-solid fa-arrow-up editRowBtn"></i>
                <i data-targetrow="${rowId}" class="arrowBtn arrowRowBtnDown fa-solid fa-arrow-down editRowBtn"></i>
            `)
        })

        $(".stepContainerCol").each(function () {
            var rowId = this.id;
            console.log(rowId)
            $(this).addClass("resizableCol");

            $(this).find(".stepColInner").append(`
                <i class="fas fa-edit editIcon"></i>
            `)

            $(this).find(".youtubeVideoItem .video-container iframe").addClass("draggableYTVideo")
        })
    ...
    $('#saveBtn').on('click', function () {
        ...
        for (var i = 0; i < stepTitles.length; i++) { 
            var stepInfo = {};
            stepInfo.StepId = stepIds[i].id;
            stepInfo.StepTitle = stepTitles[i].value;
        ...
        if (!isEdit) {            
            ...
            $.ajax({
                ...
                data: { sc: supplyChain, steps: steps },
                success: function (response) {
    ...
    $('#btnAddRow').on('click', function (e) {
        ...
        var heightOfTitle = $("#titleText").outerHeight(true);

        rowsInActiveTab.each(function () {
            heightOfElements += $(this).outerHeight(true);
            console.log($(this).outerHeight(true))
        })
        console.log(heightOfElements)

        heightOfElements += heightOfTitle; 
        console.log($('#stepsContainerMain').outerHeight(true) - heightOfElements)

        var remainingSpace = $('#stepsContainerMain').outerHeight(true) - heightOfElements - 12;
        if ((heightOfElements + 95) < $('#stepsContainerMain').outerHeight(true)) {

            $('#' + activeTabId).append(`
            <div id="stepRow${rowCount}" class="row stepContainerRow resizableRow" data-xheight="85">
        ...
        }//default height of row minus border
        else if (remainingSpace > 63) {
            $('#' + activeTabId).append(`
            <div id="stepRow${rowCount}" style="height: ${remainingSpace}px" class="row stepContainerRow resizableRow" data-xheight="${remainingSpace}">
                <i data-targetrow="stepRow${rowCount}" class="addToRowBtn fa fa-plus-circle editRowBtn"></i>
                <i data-targetrow="stepRow${rowCount}" class="rowDeleteBtn fa-solid fa-trash-can editRowBtn"></i>
                <i data-targetrow="stepRow${rowCount}" class="arrowBtn arrowRowBtnUp fa-solid fa-arrow-up editRowBtn"></i>
                <i data-targetrow="stepRow${rowCount}" class="arrowBtn arrowRowBtnDown fa-solid fa-arrow-down editRowBtn"></i>        
    ...
    $('#btnAddElementItem').on('click', function (e) {
        ...
        if (currentEditing == 1) {
            ...
            $(`#ele${elementNumber}`).html(textareaValue + '<i class="fas fa-edit editIcon"></i>')
            $(`#ele${elementNumber}`).addClass("textItem");
        }
        else if (currentEditing == 2) {            
            $(`#ele${elementNumber}`).html(`<img class="step-img resizableImg" src="${currentImageUrl}" />` + '<i class="fas fa-edit editIcon"></i>')
            $(`#ele${elementNumber}`).addClass("imageItem")
            ...
            if ($('.ytvideoEditorView').is(':visible')) {                
                ...
                $(`#ele${elementNumber}`).html(videoHtml + '<i class="fas fa-edit editIcon"></i>')
                $(`#ele${elementNumber}`).addClass("videoItem youtubeVideoItem")
                ...
                $(`#ele${elementNumber}`).html(videoHtml + '<i class="fas fa-edit editIcon"></i>')
                $(`#ele${elementNumber}`).addClass("videoItem")
    ...
    $('#btnUpdateItem').on('click', function (e) {
        ...
        if (currentEditing == 1) {
            ...
            currentlyEditingItem.parent().html(textareaValue + '<i class="fas fa-edit editIcon"></i>')
            currentlyEditingItem.parent().addClass("textItem")
        }
        else if (currentEditing == 2) {
            currentlyEditingItem.parent().html(`<img class="step-img" src="${currentImageUrl}" />` + '<i class="fas fa-edit editIcon"></i>')
            currentlyEditingItem.parent().addClass("imageItem")
            ...
            if ($('.ytvideoEditorView').is(':visible')) {
                ...
                currentlyEditingItem.parent().html(videoHtml + '<i class="fas fa-edit editIcon"></i>');
                currentlyEditingItem.parent().addClass("videoItem youtubeVideoItem")
                ...
                currentlyEditingItem.parent().html(videoHtml + '<i class="fas fa-edit editIcon"></i>')
                currentlyEditingItem.parent().addClass("videoItem")
    ...
    function GetSupplyChainData() {
        ...
        scHtml.find(".editRowBtn").remove();
        scHtml.find(".editIcon").remove();
        scHtml.find(".resizableCol").removeClass('resizableCol');
        scHtml.find(".resizableRow").removeClass('resizableRow');
        scHtml.find(".draggableYTVideo").removeClass('draggableYTVideo');

        scHtml.find('.nav-link').removeClass('active');
        scHtml.find('.tab-pane').removeClass('active');

        var firstTabBody = $('.tabItem').first().data('bs-target');

        scHtml.find('.tabItem:first').addClass("active");
        scHtml.find(firstTabBody).addClass("active");
...
`
    ...
    function LoadItemEditor(currentlyEditingItem) {
        ...
        if (currentlyEditingItem == "title") {
            ...
            if (currentlyEditingItem.parent().hasClass('textItem')) {
                ...
                innerText = innerText.replace('<i class="fas fa-edit editIcon"></i>', '')
                $('#summernote').summernote('reset')
                $('#summernote').summernote('code', innerText);
            }
            else if (currentlyEditingItem.parent().hasClass('imageItem')) {
                currentEditing = 2;
                $('#imageEditorView').show();
            }
            else if (currentlyEditingItem.parent().hasClass('videoItem')) {
                //$('#btnAddElementItem').attr('disabled', false);
                currentEditing = 3;
                $('#videoEditorView').show();
            }
                ...
                .resizable({
                    ...
                    listeners: {
                        move: function (event) {
                            ...
                            var rowsInActiveTab = $('#' + activeTabId).find('.stepContainerRow')
                            var heightOfElements = $("#titleText").outerHeight(true);
                            ...
                            var currHeight = $(event.target).height();
                            var heightChange = event.rect.height - currHeight

                            heightOfElements += heightChange;
                            console.log(heightOfElements)

                            if (heightOfElements > $('#stepsContainerMain').outerHeight(true)) {
                                console.log("Max")

                                var over = heightOfElements - $('#stepsContainerMain').outerHeight(true)
                                event.rect.height = event.rect.height - over;
                                sweetAlert("Maximum space used for this tab")
                            }
